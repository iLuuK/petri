...
class OmenivorLive(BehaviorLive):
    __energyStart = 100
    __energyReproduce = 50
    __energyWait = 1
    __energyMove = 2
    __baseColor = Color(0, 0, 135)
    __nbMove = 2
    ...
    def __reproduce(self):
        if self.getCell().getIsAlive() and self.getCell().canAction(self.__energyReproduce):
            roproducePosition = self.getCell().getPetri().canFeed(self.getCell().getX(), self.getCell().getY(),
                                                             [CellType.CARNIVOROUS])
            if roproducePosition:
                if roproducePosition[0] == self.getCell().getX() and roproducePosition[1] == self.getCell().getY():
                    reproductivePartner = self.getCell().getPetri().getCell(roproducePosition[0], roproducePosition[1])
                    if reproductivePartner.canAction(self.__energyReproduce):
                        self.getCell().setEnergy(int(self.getCell().getEnergy() / 2))
                        self.__wait()
                        self.__randomMove()

    def __goToRoproduce(self):
        roproducePosition = self.getCell().getPetri().canFeed(self.getCell().getX(), self.getCell().getY(),
                                                         [CellType.CARNIVOROUS])
        if roproducePosition and self.getCell().canAction(self.__energyReproduce) and self.canGo(roproducePosition[0],
                                                                                       roproducePosition[1]):
            newPositionX = self.getCell().getX()
            newPositionY = self.getCell().getY()

            if self.getCell().getX() == roproducePosition[0]:
                if abs(self.getCell().getY() - roproducePosition[1]) < 5:
                    if self.getCell().getY() - roproducePosition[1] < 0:
                        newPositionY += 1
                    else:
                        newPositionY -= 1
                else:
                ...
                if abs(self.getCell().getX() - roproducePosition[0]) < 5:
                    if (self.getCell().getX() - roproducePosition[0] < 0):
                        newPositionX += 1
                    else:
                        newPositionX -= 1
                else:
            ...
            if futureCell is not None and self.canGo(newPositionX, newPositionY):
                if futureCell.getType() != CellType.CARNIVOROUS:
                    return False
    ...
    def __feed(self):
        feedPosition = self.getCell().getPetri().canFeed(self.getCell().getX(), self.getCell().getY(), [CellType.HERBIVOR, CellType.GRASS])
    ...
    def __goToFeed(self):
        ...
        if feedPosition and self.getCell().canAction(self.__energyMove) and self.canGo(feedPosition[0],
            ...
            #while nbAcualMove < self.__nbMove:
            ...
            futureCell = self.getCell().getPetri().getCell(newPositionX, newPositionY)
            if futureCell is not None and self.canGo(newPositionX, newPositionY):
                if futureCell.getType() != CellType.HERBIVOR and futureCell.getType() != CellType.GRASS:
                    return False
                #nbAcualMove += 1

            newCell = Cell(self.getCell().getPetri(), OmenivorLive(), self.getCell().getBirthStep() + 1,
                           CellType.CARNIVOROUS)
            newCell.setEnergy(self.getCell().getEnergy() - self.__energyMove)
            newCell.setColor(Color(0, 0, 135))
    ...
    def __wait(self):
        ...
        newCell = Cell(self.getCell().getPetri(), OmenivorLive(), self.getCell().getBirthStep() + 1, CellType.HERBIVOR)
